# [Programming Guide](https://ccl.northwestern.edu/netlogo/docs/programming.html#programming-guide)
...
You can also use  [`of`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#of)  with a more complicated expression than just a variable name, for example:
...
Use  [`tick`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#tick)  at the end of your go procedure.
...
You define turtle breeds using the  [`breed`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#breed)  keyword, at the top of the Code tab, before any procedures:
...
Suppose you wanted to change the third item that’s nested inside item 3 from -2 to 9? The key is to realize that the name that can be used to call the nested list [3 0 -2] is  `item 3 mylist`. Then the  [`replace-item`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#replace-item)  reporter can be nested to change the list-within-a-list. The parentheses are added for clarity.
...
[`map`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#map)  reports a list containing the results of applying the reporter to each item in the input list. Again, use the variable named in the anonymous procedure (`x`  in the examples below) to refer to the current item in the list.
...
The  [`shapes`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#shapes)  primitive reports a list of currently available turtle shapes in the model. This is useful if, for example, you want to assign a random shape to a turtle:
...
### [Choosing a mode](https://ccl.northwestern.edu/netlogo/docs/programming.html#choosing-a-mode)
...
Before you can plot, you need to create one or more plots in the Interface tab. For more information on using and editing plots in the Interface tab, see the  [Interface Guide](https://ccl.northwestern.edu/netlogo/docs/interfacetab.html#plots).
...
A few primitives are specific to strings, such as  [`is-string?`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#is-of-type),  [`substring`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#subliststring), and  [`word`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#word):
...
NetLogo movies are exported as H.264-encoded MP4 files. To play an MP4 movie, you can use  [the VLC Player](http://www.videolan.org/vlc/index.html), a free download from the VideoLAN organization.
...
Here’s some turtles which have made a drawing over a grid of randomly shaded patches. Notice how the turtles cover the lines and the lines cover the patch colors. The  [`pen-size`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#pen-size)  used here was 2:![color chart](https://ccl.northwestern.edu/netlogo/docs/images/programming/drawing.gif)

The  [`stamp`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#stamp)  command lets a turtle leave an image of itself behind in the drawing and  [`stamp-erase`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#stamp-erase)  lets it remove the pixels below it in the drawing.

To erase the whole drawing, use the observer commmand  [`clear-drawing`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#clear-drawing). (You can also use  [`clear-all`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#clear-all), which clears everything else too.)

**Importing an image**

The observer command  [`import-drawing`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#import-drawing)  command allows you to import an image file from disk into the drawing.

[`import-drawing`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#import-drawing)  is useful only for providing a backdrop for people to look at. If you want turtles and patches to react to the image, you should use  [`import-pcolors`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#import-pcolors)  or  [`import-pcolors-rgb`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#import-pcolors-rgb)  instead.
...
Wrapping also affects how the view looks when you are following a turtle. On a torus, wherever the turtle goes, you will always see the whole world around it:![Screenshot](https://ccl.northwestern.edu/netlogo/docs/images/programming/torus.jpg)

Whereas in a box or cylinder the world has edges, so the areas past those edges show up in the view as gray:![Screenshot](https://ccl.northwestern.edu/netlogo/docs/images/programming/box.jpg)
...
can-move? merely returns true if the position  _distance_  in front of the turtle is inside the NetLogo world, false otherwise. In this case, if the turtle is at the edge of the world it simple goes back the way it came. You can also use  `patch-ahead 1 != nobody`  in place of  [`can-move?`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#can-move). If you need to do something smarter that simply turning around it may be useful to use  [`patch-at`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#patch-at)  with  [`dx`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#dxy)  and  [`dy`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#dxy).
...
There are two link designations: undirected and directed. A directed link is  _out_  of, or  _from_, one node and  _into_, or  _to_, another node. The relationship of a parent to a child could be modeled as a directed link. An undirected link appears the same to both nodes, each node has a link  _with_  another node. The relationship between spouses, or siblings, could be modeled as an undirected link.

There is a global agentset of all links, just as with turtles and patches. You can create undirected links using the  [`create-link-with`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#create-link)  and  [`create-links-with`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#create-link)  commands; and directed links using the  [`create-link-to`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#create-link),  [`create-links-to`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#create-link),  [`create-link-from`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#create-link), and  [`create-links-from`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#create-link)  commands. Once the first link has been created directed or undirected, all unbreeded links must match (links also support breeds, much like turtles, which will be discussed shortly); it’s impossible to have two unbreeded links where one is directed and the other is undirected. A runtime error occurs if you try to do it. (If all unbreeded links die, then you can create links of that breed that are different in designation from the previous links.)

In general, link primitive names indicate what kind of links they deal with:

-   Primitives that have “out” in their name utilize outgoing and undirected links. You can think of these as “the links I can use to get  _from_  the current node  _to_  other nodes.” In general, these are probably the primitives you want to use.
-   Primitives that have “in” in their name utilize incoming and undirected links. You can think of these as “the links I can use to get  _to_  the current node  _from_  other nodes.”
-   Primtives that do not specify “in” or “out”, or have “with” in their name utilize  _all_  links, both undirected and directed, incoming and outgoing.
...
Link breeds, like turtle breeds, allow you to define different types of links in your model. Link breeds must either be directed or undirected, unlike unbreeded links this is defined at compile time rather than run time. You declare link breeds using the keywords  [`undirected-link-breed`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#undirected-link-breed)  and  [`directed-link-breed`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#directed-link-breed). Breeded links can be created using the commands  [`create-<breed>-with`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#create-link)  and  [`create-<breeds>-with`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#create-link)  for undirected breeds and the commands  [`create-<breed>-to`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#create-link),  [`create-<breeds>-to`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#create-link),  [`create-<breed>-from`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#create-link), and  [`create-<breeds>-from`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#create-link)  for directed links.
...
As part of our network support we have also added several different primitives that will help you to visualize the networks. The simplest is  [`layout-circle`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#layout-circle)  which evenly spaces the agents around the center of the world given a radius.![Screenshot](https://ccl.northwestern.edu/netlogo/docs/images/programming/circle-layout.gif)

[`layout-radial`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#layout-radial)  is a good layout if you have something like a tree structure, though even if there are some cycles in the tree it will still work, though as there are more and more cycles it will probably not look as good.  [`layout-radial`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#layout-radial)  takes a root agent to be the central node places it at (0,0) and arranges the nodes connected to it in a concentric pattern. Nodes one degree away from the root will be arranged in a circular pattern around the central node and the next level around those nodes and so on.  [`layout-radial`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#layout-radial)  will attempt to account for asymmetrical graphs and give more space to branches that are wider.  [`layout-radial`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#layout-radial)  also takes a breed as an input so you use one breed of links to layout the network and not another.![Screenshot](https://ccl.northwestern.edu/netlogo/docs/images/programming/radial-layout.gif)

Given a set of anchor nodes  [`layout-tutte`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#layout-tutte)  places all the other nodes at the center of mass of the nodes it is linked to. The anchor set is automatically arranged in a circle layout with a user defined radius and the other nodes will converge into place (this of course means that you may have to run it several times before the layout is stable.)![Screenshot](https://ccl.northwestern.edu/netlogo/docs/images/programming/tutte-layout.gif)

[`layout-spring`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#layout-spring)  is useful for many kinds of networks. The drawback is that is relatively slow since it takes many iterations to converge. In this layout the links act as springs that pull the nodes they connect toward each other and the nodes repel each other. The strength of the forces is controlled by inputs to the primitives. These inputs will always have a value between 0 and 1; keep in mind that very small changes can still affect the appearance of the network. The springs also have a length (in patch units), however, because of all the forces involved the nodes will not end up exactly that distance from each other.
...
### [Anonymous procedure primitives](https://ccl.northwestern.edu/netlogo/docs/programming.html#anonymous-procedure-primitives)
...
### [Anonymous procedure inputs](https://ccl.northwestern.edu/netlogo/docs/programming.html#anonymous-procedure-inputs)
...
### [Anonymous procedures and strings](https://ccl.northwestern.edu/netlogo/docs/programming.html#anonymous-procedures-and-strings)
...
### [Concise syntax](https://ccl.northwestern.edu/netlogo/docs/programming.html#concise-syntax)
...
### [Anonymous procedures as closures](https://ccl.northwestern.edu/netlogo/docs/programming.html#anonymous-procedures-as-closures)
...
### [Nonlocal exits](https://ccl.northwestern.edu/netlogo/docs/programming.html#nonlocal-exits)
...
### [Anonymous procedures and extensions](https://ccl.northwestern.edu/netlogo/docs/programming.html#anonymous-procedures-and-extensions)

The extensions API supports writing primitives that accept anonymous procedures as input. Write us for sample code.

### [Limitations](https://ccl.northwestern.edu/netlogo/docs/programming.html#limitations)
...
### [What is Optional?](https://ccl.northwestern.edu/netlogo/docs/programming.html#what-is-optional)

There are several different ways of writing anonymous procedures which allow users to omit part or all of the anonymous procedure syntax. These are summarized in the table below.
...
### [Code example](https://ccl.northwestern.edu/netlogo/docs/programming.html#code-example)
...
In very old versions of NetLogo,  [`ask`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#ask)  had simulated concurrent behavior by default. Since NetLogo 4.0 (2007),  [`ask`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#ask)  is serial, that is, the agents run the commands inside the ask one at a time.

The following information describes the behavior of the  [`ask-concurrent`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#ask-concurrent)  command, which behaves the way the old  [`ask`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#ask)  behaved.

[`ask-concurrent`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#ask-concurrent)  produces simulated concurrency via a mechanism of turn-taking. The first agent takes a turn, then the second agent takes a turn, and so on until every agent in the asked agentset has had a turn. Then we go back to the first agent. This continues until all of the agents have finished running all of the commands.
...
The  [`forward`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#forward)  ([`fd`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#forward)) and  [`back`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#back)  ([`bk`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#back)) commands are treated specially. When used inside  [`ask-concurrent`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#ask-concurrent), these commands can take multiple turns to execute. During its turn, the turtle can only move by one step. Thus, for example,  `fd 20`  is equivalent to  `repeat 20 [ fd 1 ]`, where the turtle’s turn ends after each run of  `fd`. If the distance specified isn’t an integer, the last fraction of step takes a full turn. So for example  `fd 20.3`  is equivalent to  `repeat 20 [ fd 1 ] fd 0.3`.

The  [`jump`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#jump)  command always takes exactly one turn, regardless of distance.

To understand the difference between  [`ask`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#ask)  and  [`ask-concurrent`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#ask-concurrent), consider the following two commands:
...
With  [`ask`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#ask), the first turtle takes five steps forward, then the second turtle takes five steps forward, and so on.

With  [`ask-concurrent`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#ask-concurrent), all of the turtles take one step forward. Then they all take a second step, and so on. Thus, the latter command is equivalent to:
...
> **Code Example:**  Ask-Concurrent Example shows the difference between  [`ask`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#ask)  and  [`ask-concurrent`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#ask-concurrent).

The behavior of  [`ask-concurrent`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#ask-concurrent)  cannot always be so simply reproduced using  [`ask`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#ask), as in this example. Consider this command:
...
In order to get the same behavior using  [`ask`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#ask), we would have to write:
...
To prolong an agent’s “turn”, use the  [`without-interruption`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#without-interruption)  command. (The command blocks inside some commands, such as  [`create-turtles`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#create-turtles)  and  [`hatch`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#hatch), have an implied  [`without-interruption`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#without-interruption)  around them.)

Note that the behavior of  [`ask-concurrent`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#ask-concurrent)  is completely deterministic. Given the same code and the same initial conditions, the same thing will always happen (if you are using the same version of NetLogo and begin your model run with the same random seed).

In general, we suggest you not use  `ask-concurrent`  at all. If you do, we suggest you write your model so that it does not depend on the exact details of how  `ask-concurrent`  works. We make no guarantees that its semantics will remain the same in future versions of NetLogo, or that it will continue to be supported at all.

## [User Interaction Primitives](https://ccl.northwestern.edu/netlogo/docs/programming.html#user-interaction-primitives)

NetLogo features several primitives which allow a model to interact with the user. These primitives include  [user-directory](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#user-directory),  [user-file](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#user-file),  [user-new-file](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#user-new-file),  [user-input](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#user-input),  [user-message](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#user-message),  [user-one-of](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#user-one-of), and  [user-yes-or-no?](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#user-yes-or-no).

These primitives differ in precisely what interaction they take with the user.  `user-directory`,  `user-file`, and  `user-new-file`  are all reporters which prompt the user to select an item from the file system and report the path of the selected item to NetLogo.  `user-yes-or-no?`,  `user-one-of`, and  `user-input`  all prompt the user to provide input in the form of text or a selection.  `user-message`  simply presents a message to the user.
...
### [What does “Halt” mean?](https://ccl.northwestern.edu/netlogo/docs/programming.html#what-does-halt-mean)

The primitives which prompt the user for input, as well as  `user-message`  all provide a “Halt” button. The effect of this button is the same for all of these primitives - it halts the model. When the model is halted all running code is stopped, including buttons and the command center. Since halting stops code in the middle of whatever it happened to be doing at the time it was halted, you may see strange results if you continue to run the model after a halt without setting it up again.
...
When a link’s  [`tie-mode`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#tie-mode)  is set to “fixed” or “free”  [`end1`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#end1)  and  [`end2`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#end2)  are tied together. If the link is directed  [`end1`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#end1)  is the “root agent” and  [`end2`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#end2)  is the “leaf agent”. That is when  [`end1`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#end1)  moves (using  [`fd`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#forward),  [`jump`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#jump),  [`setxy`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#setxy), etc.)  [`end2`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#end2)  also moves the same distance and direction. However when  [`end2`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#end2)  moves it does not affect  [`end1`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#end1).
...
When the root turtle turns right or left, the leaf turtle rotates around the root turtle the same amount as if a stiff were attaching the turtles. When  [`tie-mode`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#tie-mode)  is set to “fixed” the heading of the leaf turtle changes by the same amount. If the  [`tie-mode`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#tie-mode)  is set to “free” the heading of the leaf turtle is unchanged.

The  [`tie-mode`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#tie-mode)  of a link can be set to “fixed” using the  [`tie`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#tie)  command and set to “none” (meaning the turtles are no longer tied) using  [`untie`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#untie)  to set the mode to “free” you need to:  `set tie-mode "free"`.

> **Code Example:**  Tie System Example

## [Multiple source files](https://ccl.northwestern.edu/netlogo/docs/programming.html#multiple-source-files)

The  [`__includes`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#includes)  keyword allows you to use multiple source files in a single NetLogo model.
...
When you open a model that uses the  [`__includes`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#includes)  keyword, or if you add it to the top of a model and hit the Check button, the includes menu will appear in the toolbar. From the includes menu you can select from the files included in this model.

When you open included files they appear in additional tabs. See the  [Interface Guide](https://ccl.northwestern.edu/netlogo/docs/codetab.html#included-files-menu)  for more details.

You can have anything in external source files (`.nls`) that you would normally put in the Code tab:  [`globals`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#globals),  [`breed`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#breed),  [`turtles-own`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#turtles-own),  [`patches-own`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#patches-own),  [`breeds-own`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#turtles-own), procedure definitions, etc. Note though that these declarations all share the same namespace. That is, if you declare a global  `my-global`  in the Code tab you cannot declare a global (or anything else) with the name  `my-global`  in any file that is included in the model.  `my-global`  will be accessible from all the included files. The same would be true if  `my-global`  were declared in one of the included files.
...
### [Notice](https://ccl.northwestern.edu/netlogo/docs/programming.html#notice)

The remainder of this section contains technical terminology which will be unfamiliar to some readers.

### [Keywords](https://ccl.northwestern.edu/netlogo/docs/programming.html#keywords)

The only keywords in the language are  [`globals`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#globals),  [`breed`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#breed),  [`turtles-own`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#turtles-own),  [`patches-own`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#patches-own),  [`to`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#to),  [`to-report`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#to-report), and  [`end`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#end), plus  [`extensions`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#extensions)  and the experimental  [`__includes`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#includes)  keyword. (Built-in primitive names may not be shadowed or redefined, so they are effectively a kind of keyword as well.)

### [Identifiers](https://ccl.northwestern.edu/netlogo/docs/programming.html#identifiers)

All primitives, global and agent variable names, and procedure names share a single global case-insensitive namespace; local names ([`let`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#let)  variables and the names of procedure inputs) may not shadow global names or each other. Identifiers may contain any Unicode letter or digit and the following ASCII characters:
...
### [Scope](https://ccl.northwestern.edu/netlogo/docs/programming.html#scope)
...
### [Comments](https://ccl.northwestern.edu/netlogo/docs/programming.html#comments)

The semicolon character introduces a comment, which lasts until the end of the line. There is no multi-line comment syntax.

### [Structure](https://ccl.northwestern.edu/netlogo/docs/programming.html#structure)

A program consists of optional declarations ([`globals`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#globals),  [`breed`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#breed),  [`turtles-own`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#turtles-own),  [`patches-own`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#patches-own),  [`<BREED>-own`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#turtles-own),  [`extensions`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#extensions)) in any order, followed by zero or more procedure definitions. Multiple breeds may be declared with separate  [`breed`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#breed)  declarations; the other declarations may appear once only.

Every procedure definition begins with  [`to`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#to)  or  [`to-report`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#to-report), the procedure name, and an optional bracketed list of input names. Every procedure definition ends with  [`end`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#end). In between are zero or more commands.

### [Commands and reporters](https://ccl.northwestern.edu/netlogo/docs/programming.html#commands-and-reporters)
...
All commands and reporters, both primitive and user-defined, take a fixed number of inputs by default. (That’s why the language can be parsed though there is no punctuation to separate or terminate commands and/or inputs.) Some primitives are variadic, that is, may optionally take a different number of inputs than the default; parentheses are used to indicate this, e.g.  `(list 1 2 3)`  (since the  [`list`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#list)  primitive only takes two inputs by default). Parentheses are also used to override the default operator precedence, e.g.  `(1 + 2) * 3`, as in other programming languages.
...
-   [`with`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#with),  [`at-points`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#at-points),  [`in-radius`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#in-radius),  [`in-cone`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#in-cone)
...
-   `<`,  `>`,  `<=`,  `>=`
-   `=`,  `!=`
-   [`and`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#and),  [`or`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#or),  [`xor`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#xor)

### [Compared to other Logos](https://ccl.northwestern.edu/netlogo/docs/programming.html#compared-to-other-logos)
...
#### [Surface differences](https://ccl.northwestern.edu/netlogo/docs/programming.html#surface-differences)
...
-   The  [`and`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#and)  and  [`or`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#or)  reporters are special forms, not ordinary functions, and they “short circuit”, that is, they only evaluate their second input if necessary.
-   Procedures can only be defined in the Code tab, not interactively in the Command Center.
-   Reporter procedures, that is, procedures that “report” (return) a value, must be defined with  [`to-report`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#to-report)  instead of  [`to`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#to). The command to report a value from a reporter procedure is  [`report`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#report), not  `output`.
-   When defining a procedure, the inputs to the procedure must be enclosed in square brackets, e.g.  `to square [x]`.
-   Variable names are always used without any punctuation: always  `foo`, never  `:foo`  or  `"foo`. (To make this work, instead of a  `make`  command taking a quoted argument we supply a  [`set`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#set)  special form which does not evaluate its first input.) As a result, procedures and variables occupy a single shared namespace.
...
#### [Deeper differences](https://ccl.northwestern.edu/netlogo/docs/programming.html#deeper-differences)
...
-   NetLogo’s  [`run`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#run)  command works on anonymous procedures and strings, not lists (since we have no “word” data type), and does not permit the definition or redefinition of procedures.
-   Control structures such as  [`if`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#if)  and  [`while`](https://ccl.northwestern.edu/netlogo/docs/dictionary.html#while)  are special forms, not ordinary functions. You can’t define your own special forms, so you can’t define your own control structures. (You can do something similar using anonymous procedures, but you must use the  `->`,  `run`, and  `runresult`  primitives for that, you cannot make them implicit.)
-   Anonymous procedures (aka function values or lambda) are true lexically-scoped closures. This feature is available in NetLogo and in modern Lisps, but not in standard Logo.
